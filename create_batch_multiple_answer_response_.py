...
def search_and_answer(query, ans1, ans2, ans3, ans4, embedding_model, vector_store_chunk, vector_store_chunk_plus_context, reranker, llama, k):
    num_retrieval = 100
...
def main():
    ...
    for index, row in question_df.iterrows():
        ...
        ids, prompt, rag_response = search_and_answer(
            query, ans1, ans2, ans3, ans4, embedding_model, KNOWLEDGE_VECTOR_DATABASE_chunk, KNOWLEDGE_VECTOR_DATABASE_chunk_plus_context, reranker, llama, k=20
        )
        #print(f"Query: {query}")
        print("----------------------------- ")
